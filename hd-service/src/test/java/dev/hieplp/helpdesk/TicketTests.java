...
class TicketTests {
    ...
    private static final Set<String> LIST_ITEM_KEYS = Set.of(
            "id", "title", "category", "priority", "status",
            "requesterId", "requesterName", "assigneeId", "createdAt", "updatedAt");
    ...
    private long createTicket(String token, String title) throws Exception {
        var result = mvc.perform(post("/tickets")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"title\":\"" + title + "\",\"description\":\"Details\","
    ...
    void seed() throws Exception {
        ...
        var other = users.findByEmail("requester2@b.co").orElseGet(() -> {
            var u = new User();
    ...
    void requesterListsOwnTicketsOnly() throws Exception {
        var result = mvc.perform(get("/tickets")
    ...
    void statusFilter() throws Exception {
        ...
        var result = mvc.perform(get("/tickets?status=open")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andReturn();
        JsonNode list = objectMapper.readTree(result.getResponse().getContentAsString());
        ...
        mvc.perform(get("/tickets?status=bogus")
                        .header("Authorization", "Bearer " + token))
    ...
    void detailHasDescriptionAndCommentsOldestFirst() throws Exception {
        var result = mvc.perform(get("/tickets/" + ownOpenId)
                        .header("Authorization", "Bearer " + login("requester@b.co")))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void requesterForeignTicketIsSame404AsMissing() throws Exception {
        ...
        var foreignResult = mvc.perform(get("/tickets/" + foreignId)
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isNotFound())
                .andReturn();
        var missingResult = mvc.perform(get("/tickets/999999")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isNotFound())
                .andReturn();
    ...
    void agentGetsAnyTicket() throws Exception {
        mvc.perform(get("/tickets/" + foreignId)
                        .header("Authorization", "Bearer " + login("agent@b.co")))
                .andExpect(status().isOk())
