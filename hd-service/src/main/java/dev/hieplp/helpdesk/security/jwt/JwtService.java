...
/**
 * Issues and parses HS256 JWTs. Claims: {@code sub} = user id, {@code role} = {@link #ROLE_CLAIM},
 * {@code exp} = now + configured TTL (8h per spec). No refresh token — the client drops the token
 * to log out.
 */
@Component
public class JwtService {
   ...
   */
  public JwtService(JwtEncoder encoder, JwtDecoder decoder, @Value("${app.jwt.ttl}") Duration ttl) {
